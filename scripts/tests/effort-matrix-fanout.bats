...
@test "Codex generation never exceeds codexCap before the wave completes" {
  _run_fanout '{"codexAvailable":true,"codexCap":2,"units":[{"name":"codex-a","leg":"codex","task":"change a","effort":"high","paths":["src/a.js"],"proofCmd":"node --check src/a.js"},{"name":"codex-b","leg":"codex","task":"change b","effort":"high","paths":["src/b.js"],"proofCmd":"node --check src/b.js"},{"name":"claude-a","leg":"claude","task":"change c","paths":["src/c.js"],"proofCmd":"node --check src/c.js"},{"name":"codex-c","leg":"codex","task":"change d","effort":"high","paths":["src/d.js"],"proofCmd":"node --check src/d.js"}]}'
...
@test "Claude units bypass the Codex cap and stay in the current wave" {
  _run_fanout '{"codexAvailable":true,"codexCap":1,"units":[{"name":"codex-a","leg":"codex","task":"change a","effort":"high","paths":["src/a.js"],"proofCmd":"node --check src/a.js"},{"name":"claude-a","leg":"claude","task":"change b","paths":["src/b.js"],"proofCmd":"node --check src/b.js"},{"name":"claude-b","leg":"claude","task":"change c","paths":["src/c.js"],"proofCmd":"node --check src/c.js"},{"name":"codex-b","leg":"codex","task":"change d","effort":"high","paths":["src/d.js"],"proofCmd":"node --check src/d.js"}]}'
...
@test "receipts are green and returned in dependency merge order" {
  _run_fanout '{"codexAvailable":true,"codexCap":2,"units":[{"name":"leaf","leg":"claude","task":"change leaf","paths":["src/leaf.js"],"proofCmd":"node --check src/leaf.js","dependsOn":["root"]},{"name":"root","leg":"codex","task":"change root","effort":"max","justification":"critical dependency root","paths":["src/root.js"],"proofCmd":"node --check src/root.js","conflicts":true},{"name":"independent","leg":"claude","task":"change independent","paths":["src/independent.js"],"proofCmd":"node --check src/independent.js"}]}'
  _json_assert "!r.error && r.result && (() => { const receipts = r.result.receipts; const root = receipts.find((receipt) => receipt.name === 'root'); const labels = r.prompts.map((entry) => entry.opts.label || ''); const isolated = r.prompts.filter((entry) => entry.opts.isolation === 'worktree'); return receipts.map((receipt) => receipt.name).join(',') === 'root,independent,leaf' && receipts.every((receipt) => receipt.green && receipt.reviewed) && root.effort === 'max' && root.justification === 'critical dependency root' && root.branch === 'unit/root' && isolated.length === 1 && isolated[0].opts.label === 'codex:generate:root' && r.prompts.some((entry) => entry.opts.label === 'verify:root' && !('isolation' in entry.opts) && entry.prompt.includes('git diff origin/main...unit/root')) && labels.indexOf('claude:generate:leaf') > labels.indexOf('verify:root'); })()"
}

@test "codexCap defaults to four and logs its source" {
  _run_fanout '{"codexAvailable":true,"units":[{"name":"codex-a","leg":"codex","task":"change a","effort":"high","paths":["src/a.js"],"proofCmd":"node --check src/a.js"}]}'
  _json_assert "!r.error && r.result.codexCap === 4 && r.logs.some((line) => line.includes('codexCap=4') && line.includes('source=default'))"
}

@test "receipt STATUS parsing ignores failure literals outside the receipt block" {
  _run_fanout '{"codexAvailable":true,"codexCap":1,"__agentOutputByLabel":{"codex:generate:codex-a":"diff fixture: STATUS: FAILURE\n--- CODEX-WORKER RECEIPT ---\nSTATUS: SUCCESS\n--- END RECEIPT ---"},"units":[{"name":"codex-a","leg":"codex","task":"change a","effort":"high","paths":["src/a.js"],"proofCmd":"node --check src/a.js"}]}'
...
@test "leaked Codex handle is cancelled before Claude degradation" {
  _run_fanout '{"codexAvailable":true,"codexCap":1,"__agentOutputByLabel":{"codex:generate:codex-a":"{\"jobId\":\"job-abcdef12\"}"},"units":[{"name":"codex-a","leg":"codex","task":"change a","effort":"high","paths":["src/a.js"],"proofCmd":"node --check src/a.js"}]}'
...
@test "dependsOn cycle throws before dispatch" {
  _run_fanout '{"codexAvailable":false,"units":[{"name":"a","leg":"claude","task":"a","paths":["a"],"proofCmd":"true","dependsOn":["b"]},{"name":"b","leg":"claude","task":"b","paths":["b"],"proofCmd":"true","dependsOn":["a"]}]}'
