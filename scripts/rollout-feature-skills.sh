#!/usr/bin/env bash
# Rollout: reconcile release-backed clarify → spec → plan → tasks → analyze skill
# surfaces in every locally attached project. Idempotent; safe to re-run.
#
# These skills remain opt-in. Attachment makes them discoverable in each native
# harness, but does not change a project's workflow until an operator invokes a
# skill. All source leaves are derived from the row's verified immutable release.
#
...
#
# Usage:
#   rollout-feature-skills.sh                 # interactive, all local registry rows
#   rollout-feature-skills.sh --dry-run       # show plan only
#   rollout-feature-skills.sh --yes           # non-interactive
#   rollout-feature-skills.sh <project-name>  # project ID or fleet/project ID
...
# native harness directories but stay opt-in — agents invoke them by name;
...
EOF
  ...
  for harness in "${row_harnesses[@]}"; do
    ...
    for skill in "${FEATURE_SKILLS[@]}"; do
      ...
      if ! printf '%s\n' "$plan" | jq -e \
        ...
        printf 'error (release lacks exact feature skill surface): %s → %s (%s, %s)\n' "$project_key" "$root" "$harness" "$skill" >&2
        return "$TRELLIS_EX_STATE"
      fi
    done
  done
  ...
  printf '== %s ==\n' "$project_key"
  if $DRY_RUN; then
    printf '  + would reconcile attachment-managed feature skill surfaces from release %s at %s\n' "$release" "$root"
    return 0
  fi
...
echo
echo "Feature skills are release-backed, local attachment-managed surfaces."
