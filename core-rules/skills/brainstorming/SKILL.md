...
**Interaction with the mandatory-pipeline gate (spec 006).** This design-gate is always-on doctrine — it binds every change regardless of the gate. The 006 gate is a *complementary*, config-gated, default-off enforcement layer that keys on **size**, not on whether you brainstormed. When `mandatory_pipeline` is enabled, the gate accepts only a **recognized form** of above-floor design: a **spec triad** (heavyweight route) or a size-capped **`/surgical`** declaration (mechanical route) — a `docs/plans/<topic>.md` doc alone does **not** satisfy it above the floor. So the weight routing tightens above the floor: a **lightweight feature** over the floor escalates to the triad (its `docs/plans` doc is no longer enough to push), while genuinely **mechanical** over-the-floor work takes `/surgical`. Do not reach for `/surgical` to skip a spec on real feature work — that is the exact dodge the gate closes; the size cap and the audit log are what keep it honest. Below the floor, and whenever the knob is off, the three-track routing above is unchanged.
...
3. **Propose 2-3 approaches with trade-offs.** Lead with your recommendation and the reasoning behind it. When the obvious 2-3 feel thin or converge on one shape, force divergence with **lenses** before settling: **inversion** (what if the opposite were true?), **constraint-removal** (what if the hardest constraint vanished?), **audience-shift** (how would a different user or persona want it?), **10x** (what if it had to scale — or shrink — by 10×?), **expert** (how would a domain specialist frame it?). Use the lenses to escape the first-obvious-answer rut, then converge back to a recommendation. (Folded from the `idea-refine` pattern.)
