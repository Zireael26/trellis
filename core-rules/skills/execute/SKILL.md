...
2. **Dispatch a subagent** to do the work for that one unit. Execute itself does not write the implementation inline — it orchestrates (see Hard refusals). When an executor node is available and the unit is execution-heavy and bounded (a large mechanical edit, a long file-by-file change), route it there instead; units needing judgment stay on the orchestrator. This is a capability gate — "does an executor node exist", never "which model am I". Dispatch mechanics, the required explicit effort tier, and every degrade path: `core-rules/references/delegation.md` and `docs/codex-routing.md`. Routing changes only *who executes* — steps 3-6 run identically on an externally-produced diff.
...
- **Per-task review (advisory, during the loop).** The pre-tick step 5 runs the canonical review cores in-body, on the just-implemented diff. Its findings inform the rest of the loop; they write no marker.
- **The `.review-done-<hash>` marker (turn-level).** Written once after the final tick, only when every in-body review that turn was critical-free — never per task, never as a silent workaround for an unresolved critical.

Core resolution, hashing, the harness-matched marker path, and the exported `TRELLIS_REVIEW_OVERRIDE=1` escape are specified in [`references/verification-step.md`](references/verification-step.md) §4. Follow it; do not duplicate the detail here.
...
- **Writes only three things, the third conditional:** the implementation diffs its dispatched subagents produce; the single checkbox / table-cell flip `tick.sh` makes to the task file; and, only when a unit deviated from the plan, one appended line in the temporary `implementation-notes.md` at the active checkout's repo root. Never the task-file prose, and never a receipt — the receipt lives in the transcript, not the file.
...
- **Harness-neutral.** Identical behavior under Claude Code, Codex, and OMP; execute runs the same checks regardless of harness.
