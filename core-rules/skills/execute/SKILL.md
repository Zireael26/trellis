...
2. **Dispatch a subagent** to do the work for that one unit. Execute itself does not write the implementation inline — it orchestrates (see Hard refusals). **When a dispatchable executor node is available (a capability gate — "does an executor node exist", never "which model am I") and the unit is execution-heavy and bounded** — a large mechanical edit, a long file-by-file change — route that unit to it via the wrapped tracked path (`agent(prompt, { agentType: 'codex:codex-rescue' })`, forced foreground + `--effort xhigh`; see `docs/codex-routing.md` §4.5 and the `codex-executor` recipe), so day-to-day builds tap the cheaper executor for the token-expensive bulk. Units needing judgment (anything with planning or review in it) stay on the orchestrator. This changes only *who executes* — the per-task verify (step 3), the advisory review of the **actual produced diff** (step 5), and the DoD receipt (step 4) are non-negotiable and run identically on a Codex-produced diff. Degrade to a Claude subagent when no executor node is present, or when a routed unit returns empty / a job handle / an error.
