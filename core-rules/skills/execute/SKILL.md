...
2. **Dispatch a subagent** to do the work for that one unit. Execute itself does not write the implementation inline — it orchestrates (see Hard refusals). **When a dispatchable executor node is available (a capability gate — "does an executor node exist", never "which model am I") and the unit is execution-heavy and bounded** — a large mechanical edit, a long file-by-file change — route that unit through the canonical blocking worker (`agent(prompt, { agentType: 'codex-worker' })`) with an explicit `effort` from the `docs/codex-routing.md §3` ladder — mechanical bulk and standard bounded implementation from a frozen spec = xhigh (medium/high are suspended 2026-07-10 and hard-rejected by the worker — see `docs/codex-routing.md §3`); an omitted tier is a dispatch error, never a default; see `docs/codex-routing.md` §4.5 and the `codex-executor` recipe. The legacy rescue forwarder is interactive-rescue-only, never a producing Workflow path. Units needing judgment (anything with planning or review in it) stay on the orchestrator. This changes only *who executes* — the per-task verify (step 3), the advisory review of the **actual produced diff** (step 5), and the DoD receipt (step 4) are non-negotiable and run identically on a Codex-produced diff. Degrade to a Claude subagent when no executor node is present, or when a routed unit returns UNAVAILABLE / FAILURE, empty output, a defensive handle assertion, or an error.
