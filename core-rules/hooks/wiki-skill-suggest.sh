#!/usr/bin/env bash
# wiki-skill-suggest.sh — PostToolUse advisory for canonical-root gotchas.md.
#
# Reads one tool event from stdin and accepts tool_input.file_path or
# tool_input.filePath. A suggestion is emitted only when that path resolves to
# the common-repository root's existing gotchas.md. Missing jq or git emits one
# stderr degradation notice and exits zero; every other unavailable or malformed
# condition fails open silently. This hook never reads wiki content, invokes a
# skill, or blocks the edit.
...
jq -nc --arg msg "$MESSAGE" '{additionalContext: $msg}' 2>/dev/null || :
exit 0
