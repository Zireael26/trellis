#!/usr/bin/env bash
# stamp-turn.sh — Claude Stop hook, single purpose.
...
# by stop-verify's four early-exit paths, and cannot break stop-verify's 22 bats
...
[ -f "$__se_lib" ] || { echo "stamp-turn: missing sibling lib at $__se_lib — re-run sync-hooks" >&2; exit 1; }
