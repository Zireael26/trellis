    ...
    if [ $_git_status -eq 0 ]; then
      ...
      while IFS= read -r -d '' _entry; do
        ...
        case "$_prev_status" in
          R*|C*)
            # Rename/copy destination is in _entry; _prev_path is the source (new path in porcelain -z order is first)
            # porcelain -z order is new then old, so old is _entry, new is _prev_path
            _old="$_entry"
            _new="$_prev_path"
...
"
      ...
      # If a rename was left without dest (should not happen), emit the source
