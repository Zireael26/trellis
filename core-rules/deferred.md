...
Rules that are **candidates** for the parent layer but haven't earned their place yet. Each was seen in only one of the two reference projects (the monorepo project, the single-app project), or was seen in both with enough variation that lifting now would force a false abstraction.
...
**Source:** the monorepo project.
**What:** on non-trivial work, present a perfectionist critique alongside a pragmatist acceptance before proposing an action.
**Why defer:** valuable in the monorepo project because features span multiple packages and trade-offs are routinely contested. Smaller projects may not benefit from the overhead.
...
**Source:** the single-app project; the polyglot monorepo (registry: "PR size budget 400/800") — n=2.
**What:** PRs should aim for ≤400 changed lines; ceiling at 800. Larger PRs require justification.
**Why defer:** the single-app project is a single Next.js app. the monorepo project is a monorepo where cross-package refactors legitimately cross 800 lines (explicitly opts out). The numbers need per-project tuning before being a parent rule.
...
**Why defer:** heavily coupled to the monorepo project's EPM (engineering program management) flow. Worth lifting eventually but not yet.
...
**Source:** the single-app project.
**What:** automated a11y scan on every build; blocks on new violations.
**Why defer:** the single-app project is a public marketing site where a11y is a launch-gate. An internal tool or API service may not need this floor. Better as a project-local rule for user-facing projects.
...
**Why defer:** the "n=0 service-shaped projects" premise is now false — the polyglot monorepo and the RAG service are both service-shaped, and the RAG service's RSC self-call bug (2026-05-02, `NEXT_PUBLIC_SITE_URL` unset → 401 HTML) is an env-drift incident the gate targets (n=1). Still short of the threshold. Re-evaluate next cycle.
...
**Source:** the portfolio site + the RAG service (n=2).
...
**Source:** the monorepo project + the portfolio site (n=2).
...
**Why defer:** both instances are config-manifest mismatches (the monorepo project `vercel.json` crons, the portfolio site `mcp.json` endpoint), but the blast radius differs (background-job implication vs route 404). Needs a third instance to confirm the shape.
...
**Source:** the monorepo project + the RAG service (n=2).
...
**Source:** the Swift project (n=1).
**What:** a security-gate stack profile for native/mobile projects. The canonical list is `web-next | web-static | web-rag-llm | monorepo-saas | unity-game` — all web- or game-shaped. `onboard-project.sh` guessed `web-next` for a Swift package with no web surface at all, which is simply wrong; the Swift project overrides to `web-static` because it at least asserts the true things (no server, no LLM endpoint).
**Why defer:** one witness. Inventing a profile from a single Swift package would bake in whatever the Swift project happens to need — and the Swift project's own shape is about to change when its Cloudflare Worker lands, at which point `web-rag-llm` may be the honest answer and the native gap narrows to the app target alone.
**Lift when:** a second and third native/mobile project (Swift, Kotlin, Flutter — `the Flutter monorepo` plans Flutter) independently need it. Note the guess in `guess_profile()` should be fixed at the same time, so onboarding stops proposing a web profile for a native project.

### Repo-root tooling globs must exclude recursive worktree pools

**Source:** the portfolio site + the single-app project + the monorepo project (n=3).
**What:** repo-root lint, test, and typecheck configuration must ignore the worktree pool with a recursive glob (for example, `.claude/**`); stale worktrees must be removed with `git worktree remove` and `git worktree prune` rather than left to contaminate root-scoped runs.
**Why defer:** all three witnesses agree on the isolation failure, but the remedy is per-tool configuration rather than a settled agent behavior. The parent worktree-isolation rule governs checkout selection; lifting a single glob spelling would force a false abstraction across tools.
**Lift when:** a shared audit or template can assert the invariant across lint, test, and typecheck configurations, so the tool-neutral requirement can be lifted without prescribing one tool's syntax.

### Gate toolchain and interpreter resolution

**Source:** the monorepo project + the RAG service + the multi-frontend app + the Swift project (n=4).
**What:** a gate must resolve and report the actual executable, version, and project-pinned environment it will run; shadowed globals, incomplete `PATH`, and absent project-local shims must be distinguishable from a project failure.
**Why defer:** the evidence supports a common failure mode, but the remedy is not yet settled between hook bootstrap, a shared doctor diagnostic, and a parent rule. Promoting prose before choosing a deterministic check risks repeating the same environment drift.
**Lift when:** a shared bootstrap or diagnostic resolves and reports toolchains for at least two gate families, giving the parent rule a stable, tool-neutral contract.
