...
Cross-cutting rules for every active personal project. Project-specific `CLAUDE.md` files extend this — they do not repeat it.
...
- Never attempt multi-file refactors in one response. Break into phases sized by a **soft, autonomy-scoped ceiling** — ~7 files at L1–L3, widening at L4/L5 where the agent runs less interactively. A safety rail, not a hard cap: `code-review-subagent` fires at ≥3 files / ≥200 lines, so review coverage scales with phase size. Complete, verify (hooks enforce), get approval per the active autonomy level, continue.
...
- Your job as orchestrator is to orchestrate: read, plan, decide, delegate, verify, synthesize. You hold the large context window to keep the whole task surface while the work happens elsewhere — not to do the work yourself. Bounded units of *execution* leave the main loop by default; you stay in the loop by reviewing what comes back. Most of the tokens on a multi-unit task should not be yours.
- **Context trigger** for delegation: the work is genuinely independent, parallelizable, and larger than a handful of tool calls — a wide multi-file investigation, an audit spanning several subsystems, a search whose breadth you cannot predict. Don't delegate context-triggered work you could finish inline, and if one subagent can do it use one rather than several.
- **Fit trigger** for delegation, sufficient on its own: the unit matches an enumerated profile trigger — a large sustained generation, a bounded unit with a pre-existing oracle, or work whose value is review by a context that did not author it. Matching is the test; deliberately **no** comparison of whether another model would do it *better* — that test almost never passes, so the work silently stays inline, the exact failure this rule prevents. "No context pressure" is also not a reason to skip it.
- Whether to spawn a subagent to check your own work depends on run length, not preference: on a short attended run don't; on a long or multi-window run verify at a declared interval with a fresh-context subagent against the spec. Deterministic gates are exempt — mechanism, not self-checking. Rationale: `core-rules/references/model-prompting-deltas.md` § Verification. Dispatch and keep working rather than blocking; intervene if one goes off track, and reuse one that already has the context rather than spawning fresh. The payoff is fresh context and wall-clock parallelism — delegation for its own sake costs both.
- When orchestrating multi-stage work, keep planning, review, and synthesis on the orchestrator. Profile, effort, family: `core-rules/references/model-routing.md` (with `gptx.enabled`, its companion allocates families: unreserved units take `gptx.routing.default_family` where that names one, under a closed reservation list, and above `gptx.routing.orchestrator_inline_max_lines` you delegate rather than edit inline). Provider selections are authoritative: surface lane failures, fail closed, never substitute the legacy `codex-worker` path. Work orders: `core-rules/references/delegation.md`.
- Compaction is a state-preservation event, not a budget event: the `save-context-log` hook fires on `PreCompact` and writes `context-log.md` — never author that file by hand. Do not stop, narrow scope, wrap up early, or propose a new session on account of context limits; the work continues. Starting a fresh window deliberately: commit first, confirm `context-log.md` is written, continue from it. On noticed degradation prefer a fresh window over `/compact` — state lives on disk and rediscovering it beats a lossy summary. Resume: `core-rules/references/loops.md`.
...
- Code-asset pairing: when a code change has a non-code companion (a checked-in generated file, a scene/prefab reference, a fixture, a binding manifest, rendered media), update it in the same commit — typecheck/build/lint cannot detect the drift; it surfaces only at runtime or via an integrity test.
...
- Receipts required. When declaring done, include the verification command you ran, its exit code, and the diff lines that prove the change. "It works" without receipts is not done. Machine-readable form, checked by Stop hooks and emitted by the `execute` skill: `<!-- dod-receipt cmd="…" exit=<int> diff="+N/-M (K files)" -->`.
...
- Follow-ups required at completion boundaries. At a spec status flip (DECIDED/SHIPPED), a PR open, or a DoD-receipt emission on a substantive unit, end with a `Follow-ups` block — items drawn ONLY from context already read this session, never new exploration. Emit `<!-- follow-ups: <count> -->` (or `none`) alongside the receipt; Stop hooks warn, non-blocking by design. Trivial turns emit nothing. Format, capture, routing: `core-rules/references/follow-ups.md`.
...
- Live teammates mean not done. A named teammate/agent pane you spawned is a held resource: engine-run workflow agents auto-terminate, named teammates do not. Call `TaskStop` in the same turn that accepts, supersedes, fails, or abandons named work — never defer teardown, never originate `shutdown_request`. An Agent name is not a `TaskOutput` task ID: wait for messages, use `SendMessage` to follow up. Naming, flat roster, teardown: `core-rules/references/delegation.md`.
...
- If a fix doesn't work after two attempts, stop. Read the whole relevant section top-down and escalate effort (`/effort xhigh`, or your harness's equivalent — `xhigh` is the ceiling; `max` buys very little for substantially more reasoning) before the next attempt: the stuck-point is where extra reasoning pays for itself. Name the assumption the evidence just falsified, and make the next attempt test a different one.
...
A token shared by several projects has ONE canonical source, exported from your shell profile and never duplicated into per-project `.env.local`; verify it works before any deploy rather than falling back to a copy found in some project's env file. Full rule, refresh, stale-copy cleanup: `core-rules/references/gotchas-operational.md` § Shared credentials.
...
- The final message of an unattended run is different: it is the operator's first look at everything you did. Write it as a re-grounding, not a continuation of your working thread — complete sentences, outcome first, terms spelled out, no shorthand or invented labels. Applies to overnight runs, cron jobs, background workflows, and any long stretch of tool calls since the operator last spoke. Any `## Decisions made (L<n>)` and `Follow-ups` blocks are part of that message, not exceptions to it.
...
Every Trellis loop — operator cron jobs, `orchestrate` fan-outs, `/loop` and `/goal` runs — declares and honors **three ceilings** and **halts on any one**: `max_iterations`, `no_progress_iterations`, `budget_ceiling_usd`. On a trip it hard-stops (never auto-continues) and emits a structured halt report; unattended loops surface the halt in their run report. Resolution, progress signals, halt behavior, token↔dollar conversion: `core-rules/loop-safety.md`.

## Advisor

When the `advisor` tool is available (auto-forwards full history; no parameters), prefer the strongest reasoning model on offer. Call it when a second opinion would change what you do next: before locking an interpretation expensive to unwind, when an approach has stopped converging, and before declaring done on a long or unattended run. Skip it on short reactive turns where tool output you just read already dictates the next action.
