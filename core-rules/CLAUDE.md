...
- When orchestrating multi-stage work, keep planning, review, and synthesis on the orchestrator and route bounded execution-heavy units to an executor node where one exists. Which profile and how much effort: `core-rules/references/model-routing.md` (where `gptx.enabled` is set, its cross-family companion adds allocation across families). Explicit provider selections stay authoritative: surface lane failures, fail closed, never silently substitute the optional legacy `codex-worker` / OpenAI Codex plugin path. Routing policy and the work-order gate: `core-rules/references/delegation.md`.
