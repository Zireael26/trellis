...
- Context budget: mostly tool schemas, cached per type — `references/delegation.md`.
...
- When orchestrating multi-stage work, keep planning, review, and synthesis on the orchestrator. Profile and effort: `core-rules/references/model-routing.md`. Provider selections are authoritative: surface lane failures and fail closed rather than substituting a different provider. Work orders: `core-rules/references/delegation.md`.
...
- Before editing an existing file, make sure you have read it this turn — the `reread-guard` hook enforces this and blocks an edit to a file you have not read. The Edit tool errors loudly on a stale `old_string`, so a routine re-read *after* editing is not needed.
...
- Confirm which checkout you are in (`git rev-parse --show-toplevel`) before any path-sensitive op: `git clean -fd` / `git checkout .` / `git commit --amend` against shared state can destroy another checkout's work. Detail: `core-rules/references/gotchas-operational.md`.

## Definition of done

- Receipts required. When declaring done, include the verification command you ran, its exit code, and the diff lines that prove the change. Machine-readable form, checked by Stop hooks and emitted by the `execute` skill: `<!-- dod-receipt cmd="…" exit=<int> diff="+N/-M (K files)" -->`.
- Receipt evidence must come from executing or diffing the artifact, never agent self-report. Where a skill has an eval, prefer a judge that runs/diffs the artifact over one that reads the transcript ([Fable Method](https://github.com/Sahir619/fable-method#results-at-a-glance)).
...
- Open todos mean not done. If `TodoWrite` has `in_progress` or `pending` items, complete them or defer/abandon with a reason. The Stop hook enforces this.
...
- Work from raw error data. Don't guess. If a bug report has no output, ask for it. Never claim anything about code you haven't opened — if the user names a file, read it before answering.
...
- If a fix doesn't work after two attempts, stop. Read the whole relevant section top-down and escalate effort (`/effort xhigh`, or your harness's equivalent — `xhigh` is the ceiling; `max` buys very little for substantially more reasoning) before the next attempt. Name the assumption the evidence just falsified, and make the next attempt test a different one.
