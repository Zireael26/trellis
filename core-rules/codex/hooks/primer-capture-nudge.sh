#!/usr/bin/env bash
# primer-capture-nudge.sh — Codex Stop. Advisory nudge for a new subsystem
# after an edit-heavy turn.
#
# Contract:
#   - stop_hook_active=true, malformed input, a clean tree, a missing INDEX, or
#     missing git all exit 0 silently. This hook never blocks a turn.
...
#     are suppressed. Unknown subsystem slugs produce a systemMessage nudge to
#     run /primer <slug>.
...
#
# Dependencies: jq (required), git (required for change discovery; optional in
# the sense that its absence is a clean advisory no-op).
#
# Source: Trellis / core-rules / codex hooks (C6, primer-capture-nudge).
...
# into a hard Stop failure; doctor/sync-codex-hooks reports missing siblings.
...
jq -nc --arg ctx "$MESSAGE" '{systemMessage: $ctx}'
exit 0
