...
- **No `tasks.md` discipline.** Parent rule `engineering-process.md §14.6` recommends the spec-kit pipeline for multi-step features. *Carve-out:* experimental work explores; specs lock in assumptions too early. Use `TodoWrite` only; skip the committed pipeline artifacts.
