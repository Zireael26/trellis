...
} from "./information-profiles.ts";
import { segmentPaletteForPreset } from "./presets/index.ts";
import {
...
const SEGMENT_DESCRIPTIONS: Record<SegmentName, string> = {
	...
	session: "Session id (first 8 characters)",
	subagents: "Live subagent count from the pi-subagents status",
	commit: "Short git HEAD commit",
	token_in: "Session input tokens",
	token_out: "Session output tokens",
	token_total: "Session tokens (input + output + cache writes)",
	token_rate: "Output tokens per second of the latest assistant message",
	cache_read: "Session prompt-cache read tokens",
	cache_write: "Session prompt-cache write tokens",
	cache_hit: "Session prompt-cache hit rate",
	context_total: "Context window size",
	time_spent: "Active agent processing time this session",
};
