...
export interface GitStatusSummary {
	root?: string;
	/** Short HEAD commit hash (omp-statusline `commit` segment). */
	commit?: string;
	ahead: number;
	behind: number;
	staged: number;
...
export async function readGitStatus(
	...
	const [statusResult, rootResult, commitResult] = await Promise.all([
		pi.exec(
			"git",
	...
	if (commitResult && commitResult.code === 0 && !commitResult.killed) {
		const commit = commitResult.stdout.split(/\r?\n/u)[0]?.trim();
		if (commit && /^[0-9a-f]{4,40}$/u.test(commit)) summary.commit = commit;
	}
	return summary;
}
