...
- **File tools** (`Read`, `Glob`, `Grep`) with access to `__TRELLIS_PATH__/` AND `__PROJECTS_ROOT__/<project>/`. Use these for all lockfile / manifest reads. The `cross-project-process-audit` audit uses the same access pattern successfully.
...
2. `Read` `__TRELLIS_PATH__/blacklist.md` (both sections).
3. Target set = `registry \ blacklist` (sections 1 + 2).
4. Read prior audit at `__TRELLIS_PATH__/audits/<previous-date>-dep-vulnerabilities.md` if present (use `Glob` to find the most recent) — used only for the "newly introduced this run" delta.
...
Write to `__TRELLIS_PATH__/audits/YYYY-MM-DD-dep-vulnerabilities.md`:
